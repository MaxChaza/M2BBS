 ...
 * used instead.
 */
public class PrismRendererTopView implements GLSurfaceView.Renderer {
	...
	private final PrismActivityWithTopView activity;
	...
	private float centreXTopView = -2f;
	 ...
	 */
	public PrismRendererTopView(final PrismActivityWithTopView prismActivityWithTopView, int nbSph, int divisions,
			float taille) {
		activity = prismActivityWithTopView;
		...
		cube = new Cube(0.0f, 0.0f, 0.0f, 0.3f, 0.7f);
	...
	public void onDrawFrame(GL10 glUnused) {
		...
		// Matrix.translateM(matrix.getmModelMatrix(), 0, cube.getFacteurX(), 0,
		// 0);
		// Matrix.scaleM(matrix.getmModelMatrix(), 0, cube.getFacteurScale(), 1,
		// 1);
		...
		if (cylindre.isVisible()) {
			...
			// Affichage de la vue aériène de droite
			if (cibleIsTouch) {
				translateAndScaleASphereTopView(cibleCoords.getX(),
						cibleCoords.getZ(), cibleCoords.getY(),
						cibleCoords.getRayon(), moyX(listCollisions), moyY(listCollisions), moyZ(listCollisions));
			...
			for (MaSphere co : listCollisions) {
				...
				// Affichage de la vue aériène de droite
				translateAndScaleASphereTopView(co.getX(), co.getZ(),
						co.getY(), co.getRayon(), moyX(listCollisions), moyY(listCollisions), moyZ(listCollisions));
				...
				if (!enCoursList.isEmpty()) {
					...
					if (!enCoursCibleList.isEmpty()) {
						if (backList.contains(cibleCoords)) {
							...
							translateAndScaleASphereTopView(cibleCoords.getX(),
									cibleCoords.getZ(), cibleCoords.getY(),
									cibleCoords.getRayon(), moyX(globalList), moyY(globalList),
									moyZ(globalList));
					...
					for (MaSphere co : listCoords) {
						if (backList.contains(co)) {
							...
							// Affichage de la vue aériène de droite
							translateAndScaleASphereTopView(co.getX(),
									co.getZ(), co.getY(), co.getRayon(),
									moyX(globalList), moyY(globalList), moyZ(globalList));
	...
	private void translateAndScaleASphereTopView(float x, float y, float z,
			float rayon, float xCentre, float yCentre, float zCentre) {
		Matrix.setIdentityM(matrix.getmModelMatrix(), 0);
		float dX = xCentre - centreXTopView;
		float rapprochement = 0.5f;
		float newX = x - dX;
		float newXRapproche = 0;
		if (newX >= centreXTopView)
			newXRapproche = newX - ((newX - centreXTopView) * rapprochement);

		if (newX < centreXTopView)
			newXRapproche = newX + ((centreXTopView - newX) * rapprochement);

		Matrix.translateM(matrix.getmModelMatrix(), 0, newXRapproche, y
				- zCentre, z - yCentre - 0.2f);
		// Tous le meme Y
//		Matrix.translateM(matrix.getmModelMatrix(), 0, newXRapproche, y
//				- zCentre, yCentre);
		
		Matrix.rotateM(matrix.getmModelMatrix(), 0, 90, 1, 0, 0);
		Matrix.scaleM(matrix.getmModelMatrix(), 0, rayon, rayon, rayon);
		// This multiplies the view matrix by the model matrix, and stores
		// the
		// result in the MVP matrix
	...
	public void setCylinder(float xpos, float ypos, float radius,
		...
		if (ypos > (activity.getActionBarHeight()
				+ activity.getStatusBarHeight() +activity.getStatusBarHeight() + (cylindre.getRadius() * 2)+(heightScreen/10))
				&& ypos < (heightScreen - (activity.getNavigationBarHeight()+(heightScreen/10)))) {
			
			float y = (((ypos - ((heightScreen/10)+activity.getActionBarHeight() + activity
					.getStatusBarHeight())) / (heightScreen
					- ((heightScreen/10)+activity.getNavigationBarHeight()) - (activity
					.getActionBarHeight() + activity.getStatusBarHeight() + (heightScreen/10)))) * -3f)
					+ 1.5f + cylindre.getOriginalRadius();
			cylindre.setY(y);
			cylindreIn.setY(y);
	...
	public float moyX(Set list) {
		float max = -1.5f;
		float min = 1.5f;
		for (Object s : list) {
			if (((MaSphere) s).getX() > max)
				max = ((MaSphere) s).getX();
			if (((MaSphere) s).getX() < min)
				min = ((MaSphere) s).getX();
		}
		float moy = (max + min) / 2;
		return moy;
	...
	public float moyY(Set list) {
		float max = -1.5f;
		float min = 1.5f;
		for (Object s : list) {
			if (((MaSphere) s).getY() > max)
				max = ((MaSphere) s).getY();
			if (((MaSphere) s).getY() < min)
				min = ((MaSphere) s).getY();
		}
		float moy = (max + min) / 2;
		return moy;
	...
	public float moyZ(Set list) {
		float max = -1.5f;
		float min = 1.5f;
		for (Object s : list) {
			if (((MaSphere) s).getZ() > max)
				max = ((MaSphere) s).getZ();
			if (((MaSphere) s).getZ() < min)
				min = ((MaSphere) s).getZ();
		}
		float moy = (max + min) / 2;
		return moy;
